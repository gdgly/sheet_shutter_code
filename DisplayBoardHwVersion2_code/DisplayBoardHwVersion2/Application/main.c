...
//      communicationModule();
		...
		communicationModuleControlBoard();
		if(((menu_gesture_flag_cyw == 0)||((menu_gesture_flag_cyw == 2)&&(gstControlBoardStatus.bits.autoManual==1)))&&(menu_gesture_flag_A007==0))//�˵�ʹ��������cyw add    //201806_Bug_No.10
	     {//cyw add
				            shoushi_cyw++;//cyw add
				            if(shoushi_cyw>=5)//=6Լ2.4ms//cyw add
				            {//cyw add
