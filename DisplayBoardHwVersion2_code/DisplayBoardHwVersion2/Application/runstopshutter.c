...
****************************************************************************/
uint8_t gRunStopState = CHECK_ALREADY_RUNNING;
uint8_t shutter_runstop_cyw=0;
uint8_t Disp_shutter_runstop_cyw=0;

uint8_t shutterRunHandler(void);
uint8_t shutterStopHandler(void);

static uint8_t RUNSTOP_cyw=0;
/****************************************************************************/
	...
	switch(gRunStopState)
	{
		case CHECK_ALREADY_RUNNING:
		{
			...
			else
			{
				...
				// Change state
				//
				gRunStopState = SEND_RUN_COMMAND;
			}
		...
		case SEND_RUN_COMMAND:
		{
			...
			if(gstUMtoCMoperational.commandRequestStatus == eINACTIVE)
			{
				...
				// Change state
				//
				gRunStopState = WAIT_FOR_RUN_COMMAND;
			}
		...
		case WAIT_FOR_RUN_COMMAND:
		{
			...
			if(gstUMtoCMoperational.commandRequestStatus == eACTIVE)
			{
				...
				if(gstUMtoCMoperational.commandResponseStatus == eSUCCESS)
				{
					...
					//
					//GrRectFIllBolymin(0, 126, 0, 63, true, true);
					GrRectFIllBolymin(0, 127, 0, 0x00, true, true);
					//
					// Display message
					//
				//	displayText("SHUTTER SYSTEM", 2, 0, false, false, false, false);
				//	displayText("RUNNING", 2, 16, false, false, false, false);
					if(gu8_language == Japanese_IDX)
					{
					displayText("シャッターシステム", 2, 0, false, false, false, false, false, false);
					...
					// Change state
					//
					gRunStopState = SWITCH_TO_HOME_SCREEN;
		...
		case SWITCH_TO_HOME_SCREEN:
		{
			  ...
			  )
			{
				...
				// Change state
				//
				gRunStopState = CHECK_ALREADY_RUNNING;
 ...
 *
 ********************************************************************************/
uint8_t shutterRunStopPaintScreen(void)
{
	//
	// Do nothing.
	//
	...
	if(gu8_language == Japanese_IDX)
	{
	displayText("RUN/STOP_キリカエ?", 2, 0, false, false, false, false,false,false);
	displayText("YES", 2, 16, false, false, false, false,false,false);
	displayText("NO", 2, 32, true, false, false, true,false,false);
	}
	else
	{
		displayText("ARE YOU SURE YOU WANT", 2, 0, false, false, false, false,false,true);
		displayText("TO INSTALL?", 2, 16, false, false, false, false,false,true);
	    displayText("YES", 2, 32, false, false, false, false,false,false);
		displayText("NO", 2, 48, true, false, false, true,false,false);
	}
	shutter_runstop_cyw = 0;
	Disp_shutter_runstop_cyw =0;
	RUNSTOP_cyw = gstControlBoardStatus.bits.runStop;
	return 0;
}
...
uint8_t shutterRunStopUp(void)
{
	if(gKeysStatus.bits.Key_Up_pressed)
	{
		...
		if(Disp_shutter_runstop_cyw == 0)
		{
			Disp_shutter_runstop_cyw = 1;
			if(gu8_language == Japanese_IDX)
			{
 ...
 *
 ********************************************************************************/
uint8_t shutterRunStopDown(void)
{
 ...
 *
 ********************************************************************************/
uint8_t shutterRunStopMode(void)
{
 ...
 *
 ********************************************************************************/
uint8_t shutterRunStopEnter(void)
{
	if(gKeysStatus.bits.Key_Enter_pressed)
	{
		gKeysStatus.bits.Key_Enter_pressed = 0;
		shutter_runstop_cyw  = Disp_shutter_runstop_cyw;
		if(shutter_runstop_cyw == 0)
		{
			psActiveFunctionalBlock = &gsMenuFunctionalBlock;
			psActiveFunctionalBlock->pfnPaintFirstScreen();
		}
		if(shutter_runstop_cyw==1)
		{
			if(RUNSTOP_cyw==0)
			{
				RUNSTOP_cyw =1;
...
uint8_t shutterRunStopTime(void)
{
	if(shutter_runstop_cyw == 1)
	{
	   if(RUNSTOP_cyw==0)
...
*********************************************************************************/
stInternalFunctions gsShutterRunStopFunctionalBlock =
{
	0,
	&gsMenuFunctionalBlock,
	shutterRunStopPaintScreen,
	shutterRunStopTime,
	shutterRunStopUp,
	shutterRunStopDown,
	shutterRunStopMode,
