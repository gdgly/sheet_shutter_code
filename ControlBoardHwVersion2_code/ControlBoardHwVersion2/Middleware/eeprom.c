                       ...
                       uint8_t ui8Count)
{
	...
	// Initialize dataWord
	dataWord.ui32DataWord = 0;

	if( ( ui8Count > MAX_SIZE ) || ( ui32Address > 0x800 ) )
	{
		gstControlProcessorFault.bits.eepromRead = 1;
		return 1;
	}
	...
	{
		gstControlProcessorFault.bits.eepromRead = 0;
		return 0;
	}
	//
                              ...
                              uint8_t ui8Count)
{
	...
	if( ( ui8Count > MAX_SIZE ) || ( ui32Address > 0x800 ) )
	{
		gstControlProcessorFault.bits.eepromProgramming = 1;
			return 1;
	}
	...
	// number of values to be written can be accommodated in first word
	//
	if(ui32WordStartAddress == ui32LastWordStartAddress)					//	Added on 29 Oct 2014. Earlier code was
		for(ui8x = ui8StartIndex, ui8y = 0; ui8x < ui8Count; ui8x++,ui8y++)	//	malfunctioning while writing data with
			dataWord.ui8DataBytes[ui8x] = *(pui8Data + ui8y );				//	address at word boundaries.
																			//	(Problem was identified while auto/manual changeover
																			//	as system was going into stop after power cycle)
	else if((ui32Address + ui8Count) < (ui32WordStartAddress + 4))
	...
	if( 0 == (ui32LastWordStartAddress - ui32WordStartAddress) )
	{
		gstControlProcessorFault.bits.eepromProgramming = 0;
		return 0;
	}
